...
def restartChar(check):
    # if check == False:
    #     animation.configure(image=framesChar[0])
    #     check == True
    for k in range(6):
        animation.configure(image=framesChar[k])
        root.after(1000)
...
def restartDis(j):
    ...
    root.after(1000,restartDis,j) #recursive after ~1 sec

# Create buttons to reset animations when pressed
Button(tabAni, text="Restart Charge Animation",command=restartChar(True)).grid(column=0,row=1,padx=30)
Button(tabAni, text="Restart Discharge Animation").grid(column=1,row=1)
